...
function showContent() {
    ...
    headline.textContent = "Teste dein Wissen: ";
    content.append(headline);

    let subline = document.createElement("h2");
    subline.textContent = "Das ultimative Quiz für clevere Köpfe!";
    ...
    for (let i = 0; i <= data.length; i++) {
        ...
        h3.textContent = data[i].question;
        content.append(h3);

        for (let j = 0; j < data[i].choice.length; j++) {
            console.log(data[i].choice[j]);
            let btn = document.createElement("button");
            btn.textContent = data[i].choice[j];
            content.append(btn);

            btn.addEventListener("click", () => {
                if (btn.innerHTML === data[i].answer) {
                    btn.style.backgroundColor = "#00ff7f";
                } else {
